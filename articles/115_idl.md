...
2. Users should research the support for `long double` with their choice of middleware and platform.
...
| sequence\<T>   | struct             | std::vector\<T>  | list        |
| sequence<T, N> | struct , size_t N | std::vector\<T>  | list        |
